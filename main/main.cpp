...
{
    if (gpio_get_level(static_cast<gpio_num_t>(CFG_BUTTON_GPIO)) != 0)
        return false;
...
static void log_frame(const radar_gaze_t *g, const mode_frame_t *mf)
{
    ...
    pos += snprintf(buf + pos, cap - pos, "$FRAME t=%lu", (unsigned long)ms);
...
extern "C" void app_main(void)
{
    ...
    while (true)
    {
        ...
        if (last_frame_id > 0 && gaze.frame_id > last_frame_id + 1)
        {
            ...
            ESP_LOGW(TAG, "$SKIP frames=%lu total=%lu", (unsigned long)skipped,
                     (unsigned long)missed_total);
        }
