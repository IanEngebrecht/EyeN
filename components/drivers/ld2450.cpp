...
static void parse_target(const uint8_t *p, ld2450_target_t *t)
{
    ...
    const float dist = sqrtf((float)t->x_mm * (float)t->x_mm + (float)t->y_mm * (float)t->y_mm);
...
esp_err_t ld2450_create(const ld2450_config_t *cfg, ld2450_dev_t **out)
{
    ...
    ld2450_dev_t *dev = static_cast<ld2450_dev_t *>(calloc(1, sizeof(*dev)));
    if (!dev)
    {
...
static esp_err_t wait_ack(ld2450_dev_t *dev, uint16_t *out_status)
{
    ...
    while (xTaskGetTickCount() < deadline)
    {
        ...
        if (state < 4)
        {
            ...
            const uint8_t expect[] = {CMD_HEADER_0, CMD_HEADER_1, CMD_HEADER_2, CMD_HEADER_3};
...
{
    static const uint8_t cmd[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x02, 0x00,
...
{
    static const uint8_t cmd[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x04, 0x00, 0xFF,
...
static esp_err_t send_config_cmd(ld2450_dev_t *dev, const uint8_t *payload, int payload_len)
{
...
esp_err_t ld2450_set_energy_threshold(ld2450_dev_t *dev, uint16_t val)
{
    if (!dev)
        return ESP_ERR_INVALID_ARG;
    uint8_t payload[4] = {0x60, 0x00, (uint8_t)(val & 0xFF), (uint8_t)((val >> 8) & 0xFF)};
...
esp_err_t ld2450_set_speed_filter(ld2450_dev_t *dev, uint16_t val)
{
    if (!dev)
        return ESP_ERR_INVALID_ARG;
    uint8_t payload[4] = {0x62, 0x00, (uint8_t)(val & 0xFF), (uint8_t)((val >> 8) & 0xFF)};
...
esp_err_t ld2450_set_hold_time(ld2450_dev_t *dev, uint16_t val)
{
    ...
    if (val > 60)
        val = 60;
    uint8_t payload[4] = {0x61, 0x00, (uint8_t)(val & 0xFF), (uint8_t)((val >> 8) & 0xFF)};
...
esp_err_t ld2450_read_frame(ld2450_dev_t *dev, ld2450_target_t targets[3], int timeout_ms)
{
    ...
    const TickType_t deadline = xTaskGetTickCount() + pdMS_TO_TICKS((uint32_t)timeout_ms);
    uint8_t b;
    ...
    int got = uart_read_bytes(dev->uart_num, payload, FRAME_DATA_LEN, pdMS_TO_TICKS(100));
