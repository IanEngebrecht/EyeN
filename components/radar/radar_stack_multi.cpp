...
static bool nearest_target(const ld2450_target_t tgt[LD2450_TARGET_SLOTS], ld2450_target_t *out)
{
    int best = -1;
...
static bool match_target(const ld2450_target_t *seed,
                         const ld2450_target_t tgt[LD2450_TARGET_SLOTS], ld2450_target_t *out)
{
...
esp_err_t radar_stack_init(void)
{
    ...
    for (int i = 1; i < s_enabled_count; ++i)
    {
        ...
        int j = i - 1;
        while (j >= 0 && s_cfg[s_pitch_order[j]].pitch_deg > s_cfg[key].pitch_deg)
        {
    ...
    for (int i = 0; i < SLOT_COUNT; ++i)
    {
        ...
        if (err != ESP_OK)
        {
            ESP_LOGE(TAG, "create %s failed: %s", s_cfg[i].name, esp_err_to_name(err));
            return err;
        }
        ESP_LOGI(TAG, "slot %-6s  pitch=%+3d°  inv=%d  uart=%d  rx=%d  tx=%d", s_cfg[i].name,
                 s_cfg[i].pitch_deg, s_cfg[i].inverted, (int)s_cfg[i].uart_num, s_cfg[i].rx_gpio,
                 s_cfg[i].tx_gpio);
    ...
    ESP_LOGI(TAG, "%d sensor(s) enabled, %d vertical band(s)", s_enabled_count, s_band_count);
    return ESP_OK;
}

ld2450_dev_t *radar_stack_get_dev(int slot)
{
    if (slot < 0 || slot >= s_enabled_count)
        return NULL;
...
esp_err_t radar_stack_update(radar_gaze_t *out)
{
    ...
    for (int i = 0; i < SLOT_COUNT; ++i)
    {
        if (!s_cfg[i].enabled || !s_slots[i].dev)
            continue;
        ...
        select_mux_if_needed(s_cfg[i].mux_channel);
        esp_err_t err = ld2450_read_frame(s_slots[i].dev, tmp, CFG_FRAME_TIMEOUT_MS);
        if (err == ESP_OK)
        {
            if (s_cfg[i].inverted)
            {
    ...
    for (int i = 0; i < SLOT_COUNT; ++i)
    {
        if (!s_cfg[i].enabled || !slot_is_current(i))
            continue;
        ld2450_target_t cand;
        if (!nearest_target(s_slots[i].targets, &cand))
            continue;
    ...
    {
        out->elevation_norm = (float)out->vertical_band / (float)(s_band_count - 1);
    }
    return ESP_OK;
}
